...
さらに、この除外パターンを追加した直後の実地確認で、ステージング領域とNAS側の双方に既に転送済みだった`db_*.sqlite3`が消えずに残り続けることが判明した。原因はrsyncの`--delete`のデフォルト挙動で、除外パターンに一致するファイルは転送先に既存であっても削除対象にならない(除外ルールを後から追加しても、追加前にコピー済みだったファイルは自動的には消えない)ためだった。ステージングへのミラー処理に`--delete-excluded`を追加し、除外ルールの変更が次回実行で自動的に反映される(手動でのフォルダ掃除が不要になる)ようにした。NASへのpush処理側には`--delete-excluded`を追加していない: そちらの除外対象(`#recycle`/`@eaDir`/`lost+found`)はSynology管理下のディレクトリで、そもそも触れてはいけないものだからである。

`--delete-excluded`を追加した直後の実地確認で、今度は肝心の`db.sqlite3`(一貫性スナップショット)自体がNASから消えるという副作用が発覚した。原因は処理順序: 従来は「(1) sqlite3 .backupでステージングにdb.sqlite3を作成」→「(2) DB以外をミラー(db.sqlite3\*を除外・--delete-excluded付き)」の順で実行していたが、(2)は自分の除外パターン`db.sqlite3*`がステージング側に存在する(手順(1)で今まさに作ったもの)ことを検知し、`--delete-excluded`によってそれを消してしまっていた。(1)と(2)の実行順序を入れ替え、削除を伴うミラー処理(2)を先に完了させてから、最後にDBスナップショット(1)を作る順序に修正した。

処理フローは以下の2段階(実行順序が重要):
```
1. rsync -a --delete --delete-excluded \
     --exclude 'db.sqlite3*' --exclude 'db_*.sqlite3' \
     --exclude 'icon_cache/' \
     /opt/vaultwarden/data/ staging/                       # DB以外をステージングへミラー(除外ルール変更を自己修復)
2. sqlite3 db.sqlite3 ".backup 'staging/db.sqlite3'"      # 一貫性スナップショット作成(手順1の後でなければならない)
