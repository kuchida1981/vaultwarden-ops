...
システムは、ユーザーが2段階認証(2FA)の手段として「メールアドレス」および「Duo Security」を選択できないようにしなければならない(SHALL)。ユーザーは「認証アプリ(TOTP)」または「FIDO2 WebAuthn」のいずれかを2段階認証の手段として選択できなければならない(SHALL)。

#### Scenario: メールアドレスは2FA手段として選択できない
- **WHEN** ユーザーがアカウント設定の2段階認証画面を開く
- **THEN** 「メールアドレス」は新規に有効化できる選択肢として表示されない

#### Scenario: Duo Securityは2FA手段として選択できない
- **WHEN** ユーザーがアカウント設定の2段階認証画面を開く
- **THEN** 「Duo Security」は新規に有効化できる選択肢として表示されない(ユーザー個人のDuoアカウントを紐付ける経路を含む)
