...
BrevoのSMTPリレーを利用したVaultwardenからのメール送信(招待・通知・パスワードヒント等)を提供する。SMTP認証情報はGoogle Secret Managerで最小権限管理し、Terraformでは生成せずGitHub Actions Secrets経由の変数として外部から受け渡す。
